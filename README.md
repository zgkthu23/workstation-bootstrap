...
只有非默认路径才写入 `paths`。hostname 必须真实且不能与其他 active host 重叠。
...
迁移只做归一化：Node、Python、CMake、Claude Code 和 Codex 各自归入唯一 bundle；Ubuntu Zotero 改为官方手动来源；Windows diagrams.net 使用当前 `JGraph.Draw` ID；v2rayN 的 macOS/Ubuntu 映射改用官方 release 来源。runtime 仅锁定 Python 3.12 与 Node LTS，其余跟随 provider stable。
